...
namespace DOL.GS.Housing
{
    public class HouseMgr
    {
        ...
        {
            List<House> houses = GameLoop.GetListForTick<House>();

            foreach (var housesInRegion in _houseList.Values)
                houses.AddRange(housesInRegion.Values);

            return houses;
        }

        public static void AddHousesInViewRange(GamePlayer player, HashSet<House> inRangeSet)
        {
            if (player.InHouse || !_houseList.TryGetValue(player.CurrentRegionID, out var housesInRegion))
                return;

            foreach (House house in housesInRegion.Values)
            {
                if (house.IsWithinRadius(player, HousingConstants.HouseViewingDistance))
        ...
        public static List<House> GetHousesCloseToSpot(ushort regionid, int x, int y, int radius)
        {
            List<House> houses = GameLoop.GetListForTick<House>();
            int radiussqrt = radius * radius;

            if (!_houseList.TryGetValue(regionid, out Dictionary<int, House> housesInRegion))
                return houses;
