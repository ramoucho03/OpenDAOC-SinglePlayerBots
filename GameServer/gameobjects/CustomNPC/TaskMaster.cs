...
namespace DOL.GS
{
	public class TaskMaster : GameNPC
	{
		...
		public override bool Interact(GamePlayer player)
		{
			...
			else SayTo(player, "You already have a task that requires completion.");
                ...
                case "labyrinthine dungeons":
                    {
                        ...
                        // Honour the player's choice. "Long corridors" = Ranged
                        // (open layouts good for archers/casters), "labyrinthine
                        // dungeons" = Melee (tight quarters). Previously the
                        // dungeon type was hardcoded by solo/group and ignored
                        // the whisper.
                        TaskDungeonMission.eDungeonType dungeonType =
                            str.ToLower() == "long corridors"
                        ...
                        if (player.Group != null)
                            mission = new TaskDungeonMission(player.Group, dungeonType);
                        else
                        {
                            mission = new TaskDungeonMission(player, dungeonType);
