    ...
    public class NecromancerPetBrain : ControlledMobBrain
    {
        ...
        private class TetherTimer : ECSGameTimerWrapperBase
        {
            ...
            protected override int OnTick(ECSGameTimer timer)
            {
                ...
                if (_playerOwner != null && _playerOwner is GamePlayer)
                    MessageToOwner(LanguageMgr.GetTranslation(_playerOwner.Client.Account.Language, "AI.Brain.Necromancer.HaveLostBondToPet"), eChatType.CT_System, (GamePlayer)_playerOwner);
                ...
                void OutOfTetherCheck()
                {
                    ...
                    if (SecondsRemaining == 10)
                        MessageToOwner(LanguageMgr.GetTranslation(_playerOwner.Client.Account.Language, "AI.Brain.Necromancer.PetTooFarBeLostSecIm", SecondsRemaining), eChatType.CT_System, (GamePlayer)_playerOwner);
                    else if (SecondsRemaining == 5)
                        MessageToOwner(LanguageMgr.GetTranslation(_playerOwner.Client.Account.Language, "AI.Brain.Necromancer.PetTooFarBeLostSec", SecondsRemaining), eChatType.CT_System, (GamePlayer)_playerOwner);
                }
            }
            ...
            private void SetShadeIconRemainingTime(int duration)
            {
                ...
                if (EffectListService.GetEffectOnTarget((GameLiving)_playerOwner, eEffect.Shade) is not NecromancerShadeECSGameEffect shadeEffect)
                    return;
