...
namespace DOL.spells
{
    ...
    public class PetLifedrainSpellHandler : LifedrainSpellHandler
    {
        ...
        {
            if (ad == null) return;
            IGamePlayer player = ((IControlledBrain) ((GameSummonedPet) Caster).Brain).GetIPlayerOwner();
            if (player == null || !player.IsAlive) return;
            int heal = ((ad.Damage + ad.CriticalDamage)*m_spell.LifeDrainReturn)/100;
            if (player.IsDiseased)
            {
                MessageToLiving((GameLiving)player, "You are diseased !", eChatType.CT_SpellResisted);
                heal >>= 1;
            }
            if (heal <= 0) return;

            heal = player.ChangeHealth((GameObject)player, eHealthChangeType.Spell, heal);
            if (heal > 0)
            {
                MessageToLiving((GameLiving)player, "You steal " + heal + " hit point" + (heal == 1 ? "." :"s."), eChatType.CT_Spell);
            } else
            {
                MessageToLiving((GameLiving)player, "You cannot absorb any more life.", eChatType.CT_SpellResisted);
            }
        }
    }
}
