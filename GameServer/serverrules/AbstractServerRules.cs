...
namespace DOL.GS.ServerRules
{
    public abstract class AbstractServerRules : IServerRules
    {
        ...
        public abstract bool IsAllowedToGroup(IGamePlayer source, IGamePlayer target, bool quiet);
        ...
        public virtual void OnNpcKilled(GameNPC killedNpc, GameObject killer)
        {
            ...
            foreach (var pair in playerCountAndDamage)
            {
                ...
                if (player is GamePlayer gPlayer)
                    killedNpc.Faction?.OnMemberKilled(gPlayer);
            }
        ...
        private static void AwardPlayerOnNpcKill(IGamePlayer playerToAward,
        ...
        public virtual void DropLoot(GameNPC killedNpc, GameObject killer, SortedSet<ItemOwnerTotalDamagePair> itemOwners)
        {
            ...
            List<GameNPC> npcsInRadius = killedNpc.GetNPCsInRadius(WorldMgr.INFO_DISTANCE);
            
            List<IGamePlayer> iGamePlayersInRadius = playersInRadius
            .Cast<IGamePlayer>()
            .Concat(npcsInRadius.OfType<IGamePlayer>())
            .ToList();

            foreach (DbItemTemplate itemTemplate in LootMgr.GetLoot(killedNpc, killer))
            {
                if (GameMoney.IsItemMoney(itemTemplate.Name))
                    CreateMoney(killedNpc, itemTemplate, itemOwners, iGamePlayersInRadius);
                else
                    CreateItem(killedNpc, itemTemplate, itemOwners, iGamePlayersInRadius);
            }

            static void CreateMoney(GameNPC killedNpc, DbItemTemplate itemTemplate, SortedSet<ItemOwnerTotalDamagePair> itemOwners, List<IGamePlayer> playersInRadius)
            {
            ...
            static void CreateItem(GameNPC killedNpc, DbItemTemplate itemTemplate, SortedSet<ItemOwnerTotalDamagePair> itemOwners, List<IGamePlayer> nearbyPlayers)
            {
            ...
            static void NotifyNearbyPlayers(GameNPC killedNpc, GameStaticItemTimed item, List<IGamePlayer> nearbyPlayers)
            {
                foreach (IGamePlayer player in nearbyPlayers)
        ...
        public virtual void OnPlayerKilled(IGamePlayer killedPlayer, GameObject killer)
        {
            if (Properties.ENABLE_WARMAPMGR && killer is IGamePlayer && killer.CurrentRegion.ID == 163)
            ...
            static void ProcessXpGainers(IGamePlayer killedPlayer,
                out double totalDamage,
                out Dictionary<IGamePlayer, EntityCountTotalDamagePair> playerCountAndDamage,
                out ItemOwnerTotalDamagePair mostDamagingPlayer,
                out Dictionary<Group, EntityCountTotalDamagePair> groupCountAndDamage,
                out ItemOwnerTotalDamagePair mostDamagingGroup)
            {
                ...
                foreach (var pair in killedPlayer.XPGainers)
                {
                    ...
                    // We only care about players in range.
                    if (pair.Key is not IGamePlayer player || player.ObjectState is not GameObject.eObjectState.Active || !player.IsWithinRadius((GameObject)killedPlayer, WorldMgr.MAX_EXPFORKILL_DISTANCE))
                        continue;
        ...
        private static void AwardPlayerOnPlayerKill(IGamePlayer playerToAward,
            GameObject killer,
            double playerTotalDamageReceived,
            IGamePlayer killedPlayer,
            Dictionary<IGamePlayer, EntityCountTotalDamagePair> playerCountAndDamage,
            Dictionary<Group, EntityCountTotalDamagePair> groupCountAndDamage,
            out bool isWorthAnything)
        {
            ...
            void RewardMoney()
            {
                ...
                if (money > 0)
                {
                    ...
                    InventoryLogging.LogInventoryAction((GameObject)killedPlayer, (GameObject)playerToAward, eInventoryActionType.Other, money);
                }
            }

            void ProcessPlayerToAwardStats(int realmPointsEarned)
            {
                GameObject killerToUse = killer is GameNPC petKiller && petKiller.Brain is IControlledBrain petKillerBrain ?  (GameObject)petKillerBrain.GetIPlayerOwner() : killer;
        ...
        private static long CalculateOutpostExperienceBonus(IGamePlayer playerToAward, long baseXpReward)
        {
            //outpost XP
            ...
            AbstractGameKeep keep = GameServer.KeepManager.GetClosestKeepToSpot(playerToAward.CurrentRegionID, new Point3D(playerToAward.X, playerToAward.Y, playerToAward.Z), OUTPOST_RADIUS);
        ...
        public virtual bool CanGenerateNews(IGamePlayer player)
        {
            if (player.Client.Account.PrivLevel > 1)
                return false;
